...
uint16_t getX(void)
{
	if (XPT2046_ACCURACY)
	{
		uint16_t x[2] = { 1, 255 };
		//while	(x[0] != x[1])
		while	(x[0] > (x[1] + delta_x) || x[1] > (x[0] + delta_x))	//wisearahna
		{
			if (XPT2046_REVERSED) { x[0] = Y(); x[1] = Y(); }
			else { x[0] = X(); x[1] = X(); }
		}
		return x[0]+adjust_x; //wisearahna
	} 
...
uint16_t getY(void)
{
	if (XPT2046_ACCURACY)
	{
		uint16_t y[2] = { 1, 255 };
		//while (y[0] != y[1])
		while	(y[0] > (y[1] + delta_y) || y[1] > (y[0] + delta_y))	//wisearahna
		{
			if (XPT2046_REVERSED) { y[0] = X(); y[1] = X(); }
			else { y[0] = Y(); y[1] = Y(); }
		}
		return y[0] - adjust_y;	//wisearahna
	}
